...
function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (isModification && formData.product_name) {
        res = await fetch("http://127.0.0.1:8000/complaints/modify", {
          method: "POST",
        ...
        res = await fetch("http://127.0.0.1:8000/complaints/analyze", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleCommit = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/complaints/commit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.duplicate) {
        alert("⚠ Duplicate Complaint Detected!\nExisting ID: " + data.existing_id);
      } else {
        alert("Complaint Saved Successfully! ID: " + data.id);
      }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/complaints/analyze-pdf", {
        method: "POST",
        body: formDataUpload
      });
  ...
  {isExpanded && (
    <>
      ...
      <div className={`risk-card severity-${formData.severity?.toLowerCase() || "default"}`}>
        ...
        <div style={{ marginTop: "16px" }}>
          <Field
            ...
            value={getValue("initial_risk_assessment")}
            textarea
          />
        </div>
      </div>
